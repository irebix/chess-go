...
describe("Holopix external candidate URL", () => {
  it("allows only the local ComfyUI view endpoint", () => {
    const value = assertHolopixCandidateUrl(
      "http://127.0.0.1:8188/view?filename=a.png&subfolder=Holopix%2FChessGo&type=output"
    );
  ...
  it("rejects other origins, paths and missing filenames", () => {
    ...
    expect(() => assertHolopixCandidateUrl("http://127.0.0.1:8188/history?filename=a.png"))
      .toThrow(/本机 ComfyUI/);
    expect(() => assertHolopixCandidateUrl("http://127.0.0.1:8188/view"))
      .toThrow(/缺少文件名/);
  });
});
