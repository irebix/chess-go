...
- Photoshop 中打开任意文档时，都会显示默认收起的一级折叠栏“AI编辑”和“AI勾线”，不要求文档具有 ChessGo 画板、参考图或布局元数据；没有文档时两栏隐藏。AI编辑当前内容为空。AI勾线直接读取当前活动图层并提交 24 节点 Centerline Forge 工作流；短边不大于 300 px 的输入先等比放大到短边 300 px，再用主体蒙版提取背景主色，并将原图与蒙版四周各扩展 20 px 后进入 Holopix 和矢量化链路。“生成描边”和使用最近就绪结果的“插入就绪描边”都输出黑色无填充 Shape 图层；
...
`release` 分支根目录包含纯 ASCII、全英文 `install.cmd`。用户可以只下载这一份 CMD：双击后安装器会自动定位或通过 winget 安装 Git，并先通过 GitHub API 比较 `release` 版本；版本一致时跳过 Git，有更新时优先拉取 Git，Git 通道不可用时自动改用 GitHub Codeload 发布压缩包。随后只需选择包含 `Photoshop.exe` 的 Photoshop 根目录。安装器会请求管理员权限、开启 UXP developer mode，并将固定发布目录联接到 `Plug-ins\ChessGo`。不依赖 Creative Cloud、UXP Developer Tool、CCX 或 UPIA。若 Photoshop 正在运行，安装后需重启一次 Photoshop。
