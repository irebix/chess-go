...
`release` 分支根目录包含全英文 `install.cmd`。用户双击后只需选择包含 `Photoshop.exe` 的 Photoshop 根目录；安装器会请求管理员权限、开启 UXP developer mode，并将发布目录联接到 `Plug-ins\ChessGo`。不依赖 Creative Cloud、UXP Developer Tool、CCX 或 UPIA。若 Photoshop 正在运行，安装后需重启一次 Photoshop。

Photoshop 2022 的独立 smoke test 仍需补做；参考图持久切换在不支持图层复合 API 的版本中会自动禁用。
...
1. 打开 Photoshop 2022 或更高版本，并启用开发者模式。
...
本次未记录耗时；Photoshop 2022 与 macOS smoke test 仍待后续阶段完成。
