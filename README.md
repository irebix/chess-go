...
- Photoshop 中打开任意文档时，都会在“AI 生成”与“生成 PSD”之间显示默认收起的一级折叠栏“AI勾线”，不要求文档具有 ChessGo 画板、参考图或布局元数据；没有文档时整栏隐藏。AI勾线直接读取当前活动图层并提交 20 节点 Centerline Forge 工作流；工作流先用主体蒙版提取背景主色，再将原图与蒙版四周各扩展 20 px 后进入 Holopix 和矢量化链路。“生成描边”和复用最近结果的“仅生成描边”都输出黑色无填充 Shape 图层；
- AI勾线顶部提供“生成描边”和“仅生成描边”；描边宽度常驻显示，精细度、折角敏感度和平滑度收入“高级选项”。高级标题与展开箭头作为一组靠左显示，展开内容与标题共用一个无内部分割线的外框；四个滑杆右侧均提供数字输入框，生成的工作路径与 Shape 图层统一命名为 `Autoline`。内部工作路径在 Shape 成功后自动清理，仅在转换失败时保留用于恢复；任务进度卡右下角提供取消入口，详细状态统一进入“运行与诊断”；
