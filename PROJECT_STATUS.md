...
| 用途 | 本地路径 | 分支 | 当前基线 |
| --- | --- | --- | --- |
| 源码、测试、文档 | `D:\Scripts\UXP\PsdArchive` | `main` | `0.5.8`（以当前分支 HEAD 为准） |
| 同事安装用运行包 | `D:\Scripts\UXP\ChessGo-Release` | `release` | `0.5.8`（以当前分支 HEAD 为准） |

远端公开仓库：`https://github.com/irebix/chess-go.git`。`main` 与 `release` 是同一远端的独立分支，不是嵌套目录；本地使用两个并列工作目录维护。发布分支不包含开发文档与源码。

## 当前发布快照

- `manifest.json` 与 `package.json` 均为 `0.5.8`。
- `main` 保存完整源码、测试和交接文档；`release` 只保存安装器及七个运行文件，不直接编辑构建产物。
- `ChessGo-Release` 保存已发布的 `0.5.8` 七个运行文件，并与源码仓库 `dist` 哈希一致。
- `0.5.8` 已完成自动化验证；Photoshop 最终交互验收继续由用户执行。

## 当前未发布改动

- 基于正式版 `0.5.8` 新增一级折叠栏“AI勾线”，位置固定在“AI 生成”下方、“生成 PSD”上方；没有当前 Photoshop 文档时整栏不渲染，打开任意普通文档或 ChessGo PSD 时都以默认收起状态出现，不依赖参考图、画板或隐藏布局元数据；切换到另一文档时重新收起。折叠标题、卡片、按钮、配色和间距复用棋子go现有 UI，不迁移 Centerline Forge 原插件的紫色独立皮肤。
...
- 当前改动保持版本 `0.5.8`，本轮只提交并推送到 `main`；未执行正式发布，也未同步或推送到 `release`。
...
- 未发布（基于 `0.5.8`）：在“AI 生成”下方加入一级折叠栏“AI勾线”，将 Centerline Forge 的当前图层读取、20 节点 ComfyUI 工作流、进度/取消、Shape 图层写入、最近结果复用及四个调节项迁入 React/TypeScript 主插件；工作流在 ComfyUI 内部按背景主色四周扩展 20 px，并在返回时补偿路径坐标。视觉复用棋子go既有控件体系，独立状态卡片合并到“运行与诊断”，生成动作置顶，高级参数合并折叠且支持滑杆/数字双向输入，运行文件清单保持七项不变。
...
- `ChessGo-Release/release` 保持正式 `0.5.8`，七个运行文件与源码 `dist` 哈希一致并已推送。
...
- 确认 AI 顶部工作流、物品链、候选数三行左右对齐且不显示额外标题；物品链显示名称与数量，长名称在窄面板中不撑宽；生成按钮、恢复入口和候选进度仍正常显示并保持原行为。
- 确认提示词编辑器显示为默认 Spectrum `sp-textarea`，输入文字为正常字形与 `15 px` 字号并能实时更新“重新生成”按钮状态；修改物品 A 后切换到物品 B，再切回 A，确认尚未提交的修改仍完整保留。Flux 与 GPT Image 2 的同一物品草稿应相互隔离；右下角把手仍可在 `86–360 px` 范围内调整高度且不会突出输入框。确认生成按钮上方、进度与矩阵之间、矩阵与提示词框之间、提示词框与重新生成按钮之间的新增间距自然且不浪费面板高度。
