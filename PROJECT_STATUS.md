...
- “AI 生成”与“当前 PSD”使用相同的自动识别条件，仅在当前文档是可识别的棋子归档 PSD 时显示。AI 面板会只读扫描当前 PSD 中画板名称、`参考图` 图层与空白智能对象，并读取既有隐藏布局元数据中的真实链条标签、成员画板 ID、行列顺序和物品中文名称；只要成员画板仍有唯一 `参考图` 就按原链条保留矩阵行，不再因为缺少或存在多个空白智能对象而删除整行。缺少空白智能对象不影响候选生成；第一次点击该行候选时，插件会在对应画板中直接创建嵌入式智能对象、按现有 PSD 的主流尺寸命名并完成 contain/居中回填。只有同画板存在多个目标时，候选列显示“空白智能对象不唯一”并禁止该节点生成和回填。候选回填期间，同文档结构事件的扫描先由 scope gate 暂存，只要替换或创建可能已经写入，结束后就最多约 `800 ms` 进行多次严格画板归属复查。普通观察与每个付费批次提交前始终使用严格扫描；生成范围按稳定 PSD 身份、画板 assetCode、画板 ID 与参考层 ID 复核，目标智能对象仅在回填时严格复核或创建，因此自动补建目标不会清空候选状态或取消同节点后续批次。名称同时用于矩阵第一行和 QwenVL“物件名字输入”，assetCode 保留在第二行；旧 PSD 没有成员名称且 Excel 不在内存时才回退 assetCode。PSD 参考缩略图由 Photoshop Imaging API 在 modal scope 内直接编码为 JPEG data URL；提交 QwenVL 时同样只读编码为 JPEG。Photoshop 25.4 实机要求连 `imaging.getPixels` 只读调用也位于 modal scope，因此预览和 QwenVL 取图共用串行 `executeAsModal` 读取队列；固定 sRGB 配置失败时自动回退到文档/工作 RGB 配置。读取过程不写入、不修改 PSD。若对应 Excel 仍在内存中，优先复用名称等表格信息。
...
- 制造整链只裁出部分物品，确认已经拆出的行保持可点击，缺失行单独失败可重试；重载插件后使用“恢复已有候选（不生成）”，确认共享 `prompt_id` 的输出仍按 `assetCode` 回到正确行，不与 Flux 历史混合。
...
- 在未保存的新 PSD 上点击生成，确认插件要求先保存且 ComfyUI 没有新增付费任务；保存后再生成。取得原图或精确恢复成功后重载插件，确认候选仍为 ready 而不是 idle，不会再次进入批量生成；若暂时没有缩略图，点击“恢复已有候选（不生成）”应只增强同一原图的 JS JPEG→RGBA 安全预览。
