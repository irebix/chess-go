...
| 用途 | 本地路径 | 分支 | 当前基线 |
| --- | --- | --- | --- |
| 源码、测试、文档 | `D:\Scripts\UXP\PsdArchive` | `main` | `0.3.3`（以 `git log -1` 为准） |
| 同事安装用运行包 | `D:\Scripts\UXP\ChessGo-Release` | `release` | `0.3.3`（以 `git log -1` 为准） |
...
- 候选原图只用于 PSD 回填；运行时在 ComfyUI 工作流末尾追加 `ImageScale → PreviewImage`，先生成真正的 `96×96` PNG，再经过尺寸硬校验后以内存 data URL 显示。
...
- Webpack production build：通过；仅有 `main.js` 体积建议警告（约 397 KiB），不是构建失败。
- `dist` 与发布仓库运行文件哈希一致。
- `main` 与 `release` 在整理本文档前均为干净工作区并与远端一致。

## 仍需人工验证

- 重启 Photoshop 使 `0.3.3` 生效；确认普通 PSD 不显示“AI 生成”，识别到棋子归档 PSD 后在“当前 PSD”和“生成 PSD”之间自动显示。
