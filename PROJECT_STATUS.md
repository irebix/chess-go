...
| 用途 | 本地路径 | 分支 | 当前基线 |
| --- | --- | --- | --- |
| 源码、测试、文档 | `D:\Scripts\UXP\PsdArchive` | `main` | 功能提交 `4f23f09`（生成参数记忆） |
| 同事安装用运行包 | `D:\Scripts\UXP\ChessGo-Release` | `release` | `8217c23`（棋子go 0.2.6） |
...
- 非标准或缺失 `assetCode` 保留提醒但不阻断整体生成；真正无可用图片的项目按校验结果处理。
...
- 在 Photoshop 中确认 `0.2.6` 首次显示 `智能对象边长 1024`、`画板间距 50`。
