...
| 用途 | 本地路径 | 分支 | 当前基线 |
| --- | --- | --- | --- |
| 源码、测试、文档 | `D:\Scripts\UXP\PsdArchive` | `main` | `0.3.5`（以 `git log -1` 为准） |
| 同事安装用运行包 | `D:\Scripts\UXP\ChessGo-Release` | `release` | `0.3.5`（以 `git log -1` 为准） |
...
- 候选方格保持 `1:1` 并直接显示安全缩略图；“外看”把固定的本机 `127.0.0.1:8188/view` 原图地址交给系统浏览器，“选用”仍下载原图并回填 PSD。安全预览失败时保留外看和选用兜底。
