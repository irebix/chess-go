...
| 用途 | 本地路径 | 分支 | 当前基线 |
| --- | --- | --- | --- |
| 源码、测试、文档 | `D:\Scripts\UXP\PsdArchive` | `main` | `0.4.0`（以 `git log -1` 为准） |
| 同事安装用运行包 | `D:\Scripts\UXP\ChessGo-Release` | `release` | `0.4.0`（以 `git log -1` 为准） |
...
- 候选方格保持 `1:1` 并直接显示安全缩略图；每行所有候选共用一个 Canvas，`32×32` 采样色块在 Canvas 内铺满为 `64×64` 方格，最多把单候选绘制调用量降为原来的四分之一，并把常驻 Canvas surface 从“每候选一个”降为“每可见行一个”。候选行使用 `IntersectionObserver` 按可视区挂载，滚动停止、窗口变化与重新进入视口时强制重绘；每行独立横向 overflow 已移除，只保留矩阵外层滚动。候选角标和可见“选用”按钮均已移除，只有已选项显示绿框。
...
- 选择候选后，插件先确认同 assetCode 只有一个目标画板与一个 `数字x数字_空白智能对象`，再在 `executeAsModal` 中替换内容；替换、缩放和平移后都会重新按 ID 查找 Layer，避免继续使用旧对象。定位优先使用自洽的 DOM `boundsNoEffects → scale(MIDDLECENTER) → translate(px)` 链；透明智能对象无有效 DOM 边界时，整条测量链统一退回 `smartObjectMore.transform`。目标框取替换前的画板 DOM 边界或空白智能对象边界，执行后允许一次误差校正，并检查最终中心偏差与四边溢出。每次回填会把文档/画板/图层 ID、嵌套路径、artboardRect、DOM bounds、transform 四角及各阶段误差写入诊断日志。
...
- Vitest：`28` 个测试文件、`108/108` 测试通过。
- Webpack production build：通过；仅有 `main.js` 体积建议警告（约 433 KiB），不是构建失败。
- `dist` 与发布仓库运行文件哈希一致。
...
- 在 Photoshop 中重载 `0.4.0`，恢复现有历史候选；连续上下滚动候选矩阵、折叠再展开 AI 栏并调整面板尺寸，确认候选会在进入视口后重绘，且不再随机空白或带动上下区域卡顿。
- 点击候选方格的中央与边缘，确认都能稳定切换；每行候选仍分别显示、只有当前候选显示绿框，并且回填只执行一次。
- 点击一个此前发生漂移的候选，确认生成图完整落在原空白智能对象/画板范围；随后导出诊断 ZIP，确认日志包含 `before-replace`、`after-replace`、`placement.plan`、`placement.final` 与 `after-placement` 几何记录。
