...
| 用途 | 本地路径 | 分支 | 当前基线 |
| --- | --- | --- | --- |
| 源码、测试、文档 | `D:\Scripts\UXP\PsdArchive` | `main` | `0.4.1`（以 `git log -1` 为准） |
| 同事安装用运行包 | `D:\Scripts\UXP\ChessGo-Release` | `release` | `0.4.1`（以 `git log -1` 为准） |
...
- 候选方格保持 `1:1` 并直接显示安全缩略图；每行所有候选共用一个 Canvas，`16×16` 采样色块在 Canvas 内铺满为 `64×64` 方格，并纵向合并相邻同色色块，单候选最多只需 256 次 `fillRect`。候选行使用 `IntersectionObserver` 按可视区挂载；滚动停止、窗口变化与重新进入视口时只命令式重绘当前挂载 Canvas，不再提升父面板 state 触发整区重渲染。每行独立横向 overflow 已移除，只保留矩阵外层滚动；候选角标和可见“选用”按钮均已移除，只有已选项显示绿框。
...
- Vitest：`28` 个测试文件、`111/111` 测试通过。
- Webpack production build：通过；仅有 `main.js` 体积建议警告（约 434 KiB），不是构建失败。
- `dist` 与发布仓库运行文件哈希一致。
...
- 在 Photoshop 中重载 `0.4.1`，恢复现有历史候选；连续上下滚动候选矩阵、折叠再展开 AI 栏并调整面板尺寸，确认候选会在进入视口后重绘，且不再随机空白或带动上下区域卡顿。
- 点击候选方格的中央与边缘，确认都能稳定切换；每行候选仍分别显示、只有当前候选显示绿框，并且回填只执行一次。
- 重新点击诊断中坐标跳变的 `c_cleaning1`，确认不再报“未找到空白智能对象”，生成图完整落在原画板范围；随后导出诊断 ZIP，确认 `target.rebased.offset` 接近 `[1732,148]`，并包含 `before-replace`、`after-replace`、`placement.plan`、`placement.final` 与 `after-placement` 几何记录。
