...
- Vitest：`42` 个测试文件、`201/201` 测试通过。
- Webpack production build：通过；`GptImage2.json` 已进入 `dist`，仅有 `main.js` 体积建议警告（约 502 KiB），不是构建失败。
...
- 确认提示词编辑器显示为默认 Spectrum `sp-textarea`，输入文字为正常字形与 `15 px` 字号并能实时更新“重新生成”按钮状态；切换候选/物品时内容正确同步，右下角把手仍可在 `86–360 px` 范围内调整高度且不会突出输入框。确认生成按钮上方、进度与矩阵之间、矩阵与提示词框之间、提示词框与重新生成按钮之间的新增间距自然且不浪费面板高度。
