...
- 缺少名称或非标准 `assetCode` 只保留提醒，不阻断其余可生成项目；输出时允许空名称。真正缺少可用图片等无法生成的项目才阻断或跳过。
...
7. 校验 `dist` 与发布仓库的 `manifest.json`、`index.html`、`main.js`、`main.js.LICENSE.txt`、`styles.css` 哈希一致，两个仓库均通过 `git diff --check`。
